...
plotcp(fit)
```
Il grafico di pruning complexity parameter (*cp*) illustra il relativo cross validation error (y-axis) per vari cp values (lower x-axis). Piccoli valori di *cp* portano ad alberi più grandi (upper x-axis). Osservando il grafico e analizzando il summar, scegliamo un cp pari a $0.02580909$

```{r}
pruned <- prune(fit, cp=0.02580909)
```
